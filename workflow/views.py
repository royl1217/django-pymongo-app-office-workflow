...
def get_current_user(request):
  ...
  if not user_id:
    return None
  return AppUser.objects(id=user_id).first()
...
def register(request):
  if request.method == "POST":
    ...
    if form.is_valid():
      ...
      employee = Employee(name=form.cleaned_data["username"])
      employee.save()
...
def login_view(request):
  if request.method == "POST":
    ...
    if form.is_valid():
      ...
      user = AppUser.objects(username=username).first()
...
def dashboard(request):
  ...
  pending = WorkflowRequest.objects(requester=employee, status="PENDING").count()
  approved = WorkflowRequest.objects(requester=employee, status="APPROVED").count()
  rejected = WorkflowRequest.objects(requester=employee, status="REJECTED").count()
  in_progress = WorkflowRequest.objects(requester=employee, status="IN_PROGRESS").count()
  completed = WorkflowRequest.objects(requester=employee, status="COMPLETED").count()
...
def request_list(request):
  ...
  my_requests = WorkflowRequest.objects(requester=employee).order_by("-created_at")
  to_approve = WorkflowRequest.objects(approver=employee, status="PENDING").order_by("-created_at")
...
def request_detail(request, pk):
  ...
  req = WorkflowRequest.objects(id=pk).first()
  if not req:
    return redirect("workflow:request_list")
...
def request_approve(request, pk):
  ...
  req = WorkflowRequest.objects(id=pk, approver=employee).first()
  if not req:
    return redirect("workflow:request_list")
